...
class DFC2020(data.Dataset):
    ...
    def __init__(self,
        ...
        if subset == "train":
            path = os.path.join(path, "toy_train")
        elif subset == "val":
            path = os.path.join(path, "toy_valid")
        elif subset == "test":
            path = os.path.join(path, "toy_test")
        s2_locations = glob.glob(os.path.join(path, "img", "*.tif"), recursive=True)
        self.samples = []
        #for s2_loc in tqdm(s2_locations, desc="[Load]"):
            #s1_loc = s2_loc.replace("_s2_", "_s1_").replace("s2_", "s1_") #can be deleted
        lc_locations = glob.glob(os.path.join(path, "msk", "*.tif"), recursive=True)
        #self.samples.append({"lc": lc_loc, "s1": s1_loc, "s2": s2_loc,
        #                         "id": os.path.basename(s2_loc)})
        for s2, lc in zip(s2_locations, lc_locations):
            self.samples.append({"s2": s2, "lc": lc, "id": os.path.basename(s2)})
        # sort list of samples
        self.samples = sorted(self.samples, key=lambda i: i['id'])
